...
export function TargetedReadingPanel({
  ...
  return (
    <div className="eco-panel flex flex-col rounded-xl border border-[var(--eco-rule)] bg-[var(--eco-panel)] shadow-xl overflow-hidden">
      <header className="flex items-center justify-between border-b border-[var(--eco-rule)] bg-black/30 px-4 py-3">
              ...
              <div
                ...
                className={`rounded-xl border p-3.5 transition ${
                  isPending
                    ...
                    : 'border-[var(--eco-rule)] bg-black/20 hover:border-[var(--eco-rule)]/80'
                }`}
              >
                  ...
                  {role === 'teacher' && isPending && (
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="rounded-md px-2.5 py-1 text-xs text-[var(--eco-cream-faint)] hover:text-rose-300"
                      >
                        Dismiss
                      </button>
                      <button
                        ...
                        disabled={processingId === item.id}
                        className="rounded-md bg-emerald-500/20 px-3 py-1 text-xs font-semibold text-emerald-300 ring-1 ring-emerald-500/40 hover:bg-emerald-500/30"
                      >
      ...
      {selectedReading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm animate-in fade-in">
          <div className="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-xl border border-[var(--eco-rule)] bg-[var(--eco-panel)] shadow-2xl overflow-hidden">
            <header className="flex items-center justify-between border-b border-[var(--eco-rule)] bg-black/40 px-5 py-3">
              <div>
            ...
            <footer className="border-t border-[var(--eco-rule)] bg-black/30 px-5 py-3 flex justify-end">
              <button
                type="button"
                onClick={() => setSelectedReading(null)}
