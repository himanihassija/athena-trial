...
import type { LearningGap, LanguageCode } from '@echosphere/shared-types';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-md animate-in fade-in duration-200">
      <div className="flex max-h-[85vh] w-full max-w-3xl flex-col rounded-2xl border border-[var(--eco-rule)] bg-[var(--eco-panel)] shadow-2xl overflow-hidden">
        {/* Header */}
        <header className="flex items-center justify-between border-b border-[var(--eco-rule)] bg-black/40 px-5 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-500/20 text-xl ring-1 ring-purple-500/40">
              👩‍🏫
            </div>
            <div>
        ...
        {gaps.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 border-b border-[var(--eco-rule)]/50 bg-black/20 px-5 py-2.5 text-xs">
            <span className="text-[11px] font-semibold text-amber-300 mr-1">Flagged Misconceptions:</span>
            {gaps.map((gap) => (
              <button
                key={gap.gapId}
                type="button"
                onClick={() => handleFocusGap(gap)}
                className="rounded-md bg-amber-500/10 px-2 py-0.5 text-[11px] text-amber-200 border border-amber-500/30 hover:bg-amber-500/20 transition"
              >
                🔍 {gap.topic}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 min-h-[16rem]">
          {messages.map((m, idx) => (
            ...
            >
              {m.role === 'athena' && (
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-tr from-amber-400 to-yellow-600 text-xs font-bold text-black">
                  A
                </div>
              )}
              <div
                className={`max-w-xl rounded-2xl px-4 py-2.5 text-xs leading-relaxed ${
                  m.role === 'student'
                    ...
                    : 'bg-black/40 text-[var(--eco-cream)]/90 border border-[var(--eco-rule)]/60'
                }`}
              >
                {m.text}
              </div>
        ...
        {/* Input Footer */}
        <form onSubmit={handleSend} className="border-t border-[var(--eco-rule)] bg-black/40 p-4">
          <div className="flex items-center gap-2">
            <input
              type="text"
              ...
              className="flex-1 rounded-xl border border-[var(--eco-rule)] bg-black/40 px-4 py-2.5 text-xs text-[var(--eco-cream)] placeholder-[var(--eco-cream-faint)]/50 focus:border-amber-400 focus:outline-none"
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="rounded-xl bg-amber-500 px-5 py-2.5 text-xs font-semibold text-black hover:bg-amber-400 disabled:opacity-50 transition"
            >
              Send
            </button>
          </div>
