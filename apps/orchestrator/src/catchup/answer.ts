...
function gatherSources(session: ClassroomSession, query: string): CatchupSource[] {
  ...
  // Was the whiteboard's cards before the shared board was dropped from this
  // merge. The workspace sticky notes carry the same thing the board did — what
  // the room has actually written down — so the grounding source survives.
...
function fallbackReply(question: string, sources: CatchupSource[]): string {
  ...
  if (/\b(miss|missed|catch up|recap|what happened|so far)\b/.test(lower)) {
    const parts = [
      ...
      workspaceNote ? `From the shared workspace: ${workspaceNote}` : null,
    ].filter(Boolean);
    if (parts.length > 0) {
  ...
  if (workspaceNote) {
    return `That is on the shared workspace: ${workspaceNote}`;
  }
